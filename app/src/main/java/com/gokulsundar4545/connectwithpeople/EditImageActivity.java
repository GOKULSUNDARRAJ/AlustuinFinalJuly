...
public class EditImageActivity extends BaseActivity implements OnPhotoEditorListener, View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences2 = getSharedPreferences("ModifiedImagePrefsfinal", Context.MODE_PRIVATE);
    ...
    private void saveImage() {
        ...
        if (imageUri != null) {
            try {
                ...
                // Create a bitmap with a background color
                Bitmap backgroundBitmap = Bitmap.createBitmap(mPhotoEditorView.getWidth(), mPhotoEditorView.getHeight(), Bitmap.Config.ARGB_8888);
                Canvas backgroundCanvas = new Canvas(backgroundBitmap);
                backgroundCanvas.drawColor(Color.WHITE); // Set the background color (e.g., white)

                // Create a bitmap from the PhotoEditorView
                Bitmap overlayBitmap = Bitmap.createBitmap(mPhotoEditorView.getWidth(), mPhotoEditorView.getHeight(), Bitmap.Config.ARGB_8888);
                Canvas overlayCanvas = new Canvas(overlayBitmap);
                mPhotoEditorView.draw(overlayCanvas);

                // Draw the overlayBitmap on top of the backgroundBitmap
                backgroundCanvas.drawBitmap(overlayBitmap, 0, 0, null);

                // Compress and write the full bitmap to the output stream
                ...
                editor.putString("modified_image_uri_final", imageUri.toString());
                editor.apply();
